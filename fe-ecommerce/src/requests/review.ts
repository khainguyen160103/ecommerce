...
export const ReviewService = {
  getByProduct: async (productId: string, page: number = 1, limit: number = 20) => {
    const res = await axiosClient.get(ENDPOINT.REVIEW.GET_BY_PRODUCT(productId), {
      params: { page, limit },
    });
    return res.data;
  },
  ...
  adminDelete: async (reviewId: string) => {
    const res = await axiosClient.delete(ENDPOINT.REVIEW.ADMIN_DELETE(reviewId));
    return res.data;
  },
};
